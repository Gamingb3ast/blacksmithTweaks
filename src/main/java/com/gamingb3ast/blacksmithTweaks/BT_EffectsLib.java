...
public class BT_EffectsLib {
    ...
    public static BT_Effect getWeightedRandomEffect() {
        ...
        for(BT_Effect effect : effects_list) {
            if (r < effect.getWeight())
                return effect;
            r -= effect.getWeight();
        }
        try {
            return getWeightedRandomEffect(); //Shouldn't stack overflow unless you're REALLY unlucky. It's one in a quadrillion. But still, I'll account for it.
        }
